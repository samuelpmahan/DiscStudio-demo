...
After selecting a circle, **Refine choice** executes the normal candidate Calculation and a separate `oc.studio.opposingRimFit` Calculation over the same photo Part. It samples multiple edges near the selected rim, fits candidate circles from opposing sides, and tests angular coverage and residuals. The largest well-supported ring appears as **Circle 3 · Rim fit**, with its hypotheses and accepted geometry in the `ds.px.OpposingRimFit.circlefit.*` evidence Part. The original choice remains Circle 1; an unsupported rim fit abstains instead of replacing it. Initial intake does not run this refinement. A fitted circle cannot perfectly follow a disc with a strongly elliptical appearance. On five real photos, the rim Calculation took 13–17 ms and the complete Refine interaction took 51–56 ms in local headless Chromium; these are measured samples, not a device-wide latency guarantee.
