...
For a bounded opposing-rim experiment, append `?rimFit=1` to the local creator URL (`?centerFit=1` remains a compatibility alias). After selecting a circle, **Refine choice** executes the normal candidate Calculation and a separate `oc.studio.opposingRimFit` Calculation over the same photo Part. It samples multiple edges near the selected rim, fits candidate circles from opposing sides, and tests angular coverage and residuals. The largest well-supported ring appears as **Circle 3 · Rim fit**, with its hypotheses and accepted geometry in the `ds.px.OpposingRimFit.circlefit.*` evidence Part. The original choice remains Circle 1; without the URL switch the experiment does not execute or change any offered crop. A fitted circle cannot perfectly follow a disc with a strongly elliptical appearance.
