...
export async function openExperience(storage: Pick<Storage, 'getItem' | 'setItem'>, log?: (event: Record<string, unknown>) => void) {
  ...
  try {
    ...
    if (raw !== null) { state = await restore(raw, createExperience(() => {}).pxc); status = 'Restored Today’s Bag · composition outputs verified.'; }
  ...
  const experience = createExperience(log, { state, status: () => status, persist(next) {
    ...
    } catch (error) { status = `Not saved locally: ${String(error)} Previous stored Today’s Bag preserved.`; throw Error(status); }
  } });
  return experience;
}
