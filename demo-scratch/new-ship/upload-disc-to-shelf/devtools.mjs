...
export function mountDevTools(pxc, { label = 'UploadDiscToShelf' } = {}) {
  ...
  function renderSaveSummary(host) {
    ...
    section.append(text('h3', summary.title));
    const created = document.createElement('p'); created.append(text('strong', 'Disc Part: '), sourceLink(summary.discAddress, summary.discAddress), text('span', ' · '), sourceLink('fn.CREATE', summary.create.calculationAddress), text('span', ' · '), verdict(summary.create.verified)); section.append(created);
    const execution = document.createElement('p'); execution.append(text('strong', (summary.kind === 'intake' ? 'Intake execution: ' : 'Save execution: ')), sourceLink(summary.readback.pqlAddress.replace('ds.px.receipt.pql.', ''), selectedLabel), text('span', ' · stage, Tick, receipt and PQL trace; the Disc Part address identifies the physical disc.')); section.append(execution);
    const bag = document.createElement('p'); bag.append(text('strong', summary.kind === 'intake' ? 'Ready discs: ' : 'Today’s Bag: '), sourceLink(String(summary.bag.beforeCount), summary.bag.beforeAddress), text('span', ' → '), sourceLink(String(summary.bag.afterCount), summary.bag.afterAddress), text('span', ' · added Part reference: '), ...(summary.bag.added.length ? summary.bag.added.flatMap(address => [sourceLink(address, address), text('span', ' ')]) : [text('span', 'none')]), verdict(summary.bag.verified)); section.append(bag);
    const flags = summary.readback.receiptFlags;
    const read = document.createElement('p'); read.append(text('strong', 'Readback: '), sourceLink('PQL', summary.readback.pqlAddress), text('span', ' · Disc '), sourceLink(summary.readback.disc.into ?? 'UNVERIFIED', summary.readback.disc.into), text('span', summary.kind === 'intake' ? ' · Intake ' : ' · Shelf '), sourceLink(summary.readback.shelf.into ?? 'UNVERIFIED', summary.readback.shelf.into), text('span', ' · flags ' + (flags.readbackMatched && (summary.kind === 'intake' ? flags.intakeContainsDisc : flags.shelfContainsDisc && flags.bagContainsDisc) ? 'confirmed' : 'incomplete') + ' · '), verdict(summary.readback.verified)); section.append(read);
